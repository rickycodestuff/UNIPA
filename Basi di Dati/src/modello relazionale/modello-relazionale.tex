...
Un altro tipo di chiave è la \textbf{chiave primaria}, un tipo di chiave che 
non accetta valori nulli, per esempio un identificativo come la 
\textit{matricola} non sarà mai nullo, quindi vale come chiave primaria.
\\
\\
Similmente al vincolo \textit{chiave} parleremo del \textbf{vincolo d'integrità
referenziale}, ovvero della \textbf{foreign key}, quest'ultima è un chiave (primaria o non)
appartenente ad una tabella A che compare anche nella tabella B ma \underline{non} come 
chiave. Di conseguenza se nella tabella A la chiave avrà valore X, quando poi la useremo
come \textit{foreign key} nella tabella B sarà vincolata ad avere il valore X.

\begin{center}
  \includegraphics[width=0.8\textwidth]{modello relazionale/foreign-key.png}
\end{center}

Come vediamo i valori delle tabelle \textit{Prov} e \textit{Numero} della tabella 
sopra compaiono \underline{identiche} nella tabella \textit{Auto}.